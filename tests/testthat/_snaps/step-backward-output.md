...
# output from stepwise backward regression matches when steps is specified
...
---
    ...
    Code
      ols_step_backward_p(model, steps = 2, progress = TRUE)
    Output
      Backward Elimination Method 
      ---------------------------
...
---
    ...
    Code
      ols_step_backward_p(model, steps = 2, details = TRUE)
    Output
      Backward Elimination Method 
      ---------------------------
      ...
      Step   => 0 
      Model  => y ~ bcs + alc_heavy + pindex + enzyme_test + liver_test + age + gender + alc_mod 
      R2     => 0.782 

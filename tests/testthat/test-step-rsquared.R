...
test_that("stepaic_forward returns the appropriate error", {
  ...
  expect_error(ols_step_rsquared(model, direction = "backward", include = c("dis")), "dis not part of the model and hence cannot be forcibly included. Please verify the variable names.")
  expect_error(ols_step_rsquared(model, direction = "backward", exclude = c("hps")), "hps not part of the model and hence cannot be forcibly excluded. Please verify the variable names.")
  expect_error(ols_step_rsquared(model, direction = "backward", include = c(5)), "Index of variable to be included should be between 1 and 4.")
  expect_error(ols_step_rsquared(model, direction = "backward", exclude = c(5)), "Index of variable to be excluded should be between 1 and 4.")
  expect_error(ols_step_rsquared(model, direction = "both", include = c("dis")), "dis not part of the model and hence cannot be forcibly included. Please verify the variable names.")
  expect_error(ols_step_rsquared(model, direction = "both", exclude = c("hps")), "hps not part of the model and hence cannot be forcibly excluded. Please verify the variable names.")
  expect_error(ols_step_rsquared(model, direction = "both", include = c(5)), "Index of variable to be included should be between 1 and 4.")
  expect_error(ols_step_rsquared(model, direction = "both", exclude = c(5)), "Index of variable to be excluded should be between 1 and 4.")
})
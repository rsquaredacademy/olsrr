...
test_that('aic output matches expected result', {
	...
	expect_equal(aic(fullmodel, method = 'R'), round(AIC(fullmodel), 4))
	expect_equal(aic(fullmodel, method = 'STATA'), 63.8367)
...
test_that('sbc output matches expected result', {
	...
	expect_equal(sbc(fullmodel, method = 'R'), round(BIC(fullmodel), 4))
	expect_equal(sbc(fullmodel, method = 'STATA'), 66.6614)
...
test_that('sbc output matches expected result', {
	...
	expect_equal(round(pc(fullmodel), 3), 0.04)

	model     <- lm(y ~ x1, data = cement)
	expect_equal(round(pc(model), 3), 0.636)

	model     <- lm(y ~ x4, data = cement)
	expect_equal(round(pc(model), 3), 0.444)

	model     <- lm(y ~ x1 + x2 + x4, data = cement)
	expect_equal(round(pc(model), 3), 0.033)

	model     <- lm(y ~ x1 + x3 + x4, data = cement)
	expect_equal(round(pc(model), 3), 0.035)

...
#'   each step.
#' @param x An object of class \code{ols_step_backward_arsq}.
#' @param print_plot logical; if \code{TRUE}, prints the plot else returns a plot object.
#' @param ... Other arguments.
#'
#' @return \code{ols_step_backward_arsq} returns an object of class \code{"ols_step_backward_arsq"}.
#' An object of class \code{"ols_step_backward_arsq"} is a list containing the
...
#' \code{ols_steparsq_backward()} has been deprecated. Instead use \code{ols_step_backward_arsq()}.
#'
#' @examples
#' # stepwise backward regression
#' model <- lm(y ~ ., data = surgical)
#' ols_step_backward_arsq(model)
#'
#' # stepwise backward regression plot
#' model <- lm(y ~ ., data = surgical)
#' k <- ols_step_backward_arsq(model)
#' plot(k)
#'
#' # final model
#' k$model
...
#' @export
#'
ols_step_backward_arsq <- function(model, ...) UseMethod("ols_step_backward_arsq")

#' @export
#' @rdname ols_step_backward_arsq
#'
ols_step_backward_arsq.default <- function(model, progress = FALSE, details = FALSE, ...) {
  ...
  preds    <- nam
  arsq_f   <- summary(model)$adj.r.squared
  ...
  while (!end) {
    ...
    if (arsq[maxc] > arsq_f) {
      ...
      rss_f <- mi$rss
      laic  <- c(laic, aics[maxc])
      lrss  <- c(lrss, rss_f)
      less  <- c(less, mi$ess)
      lrsq  <- c(lrsq, mi$rsq)
      larsq <- c(larsq, arsq_f)
      aics  <- c()
      ess   <- c()
      rss   <- c()
      rsq   <- c()
      ...
      if (details) {
        ...
        cat(rep("-", w), sep = "", "\n")
        cat(
          fl("Variable", w1), fs(), fc("DF", w2), fs(), fc("ARSQ", w3), fs(),
          fc("Sum Sq", w4), fs(), fc("RSS", w5), fs(), fc("R-Sq", w6), fs(),
          fc("Adj. R-Sq", w7), "\n"
        )
        cat(rep("-", w), sep = "", "\n")
  ...
  class(out) <- "ols_step_backward_arsq"
...
#' @export
#'
print.ols_step_backward_arsq <- function(x, ...) {
  if (x$steps > 0) {
    print_steparsq_backward(x)
  } else {
...
#' @export
#'
plot.ols_step_backward_arsq <- function(x, print_plot = TRUE, ...) {
  ...
  p <-
    ...
    ggtitle("Stepwise Adj. R-Sq Backward Elimination") +
...
#' @usage NULL
#'
ols_steparsq_backward <- function(model, details = FALSE, ...) {
  .Deprecated("ols_step_backward_arsq()")
}

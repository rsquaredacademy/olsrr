...
ols_plot_diagnostics <- function(model, print_plot = TRUE) {
  ...
  d1 <- rvspdata(model)
  p1 <-
    ggplot(d1, aes(x = predicted, y = resid)) +
  ...
  d2 <-
    ggplot(d22, aes(x = pred, y = dsr, label = txt)) +
  ...
  d3 <-
    ggplot(d33, aes(leverage, rstudent, label = txt)) +
  ...
  p5 <-
    ...
    geom_segment(aes(x = min(x), y = min(y), xend = max(x), yend = max(y)), colour = "red") +
  ...
  p10 <-
    ggplot(d10, aes(x = factor(0), y = resid)) +
